...
public class OfferDrawButton extends Button {
	...
	public OfferDrawButton(ChessBoardContainer container, int widthIn, int heightIn) {
		super(widthIn, heightIn, 20, 20, "", (button) -> {
			...
			if(container.tile.playing && !container.tile.waitingForChallenged && container.tile.challenger.isPresent() && container.tile.challenged.isPresent()) {
	...
	public void renderButton(int p_renderButton_1_, int p_renderButton_2_, float p_renderButton_3_) {
		...
		if(!this.container.tile.playing || container.tile.waitingForChallenged || this.container.tile.isPlayingComputer)
